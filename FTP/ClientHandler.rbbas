...
Inherits FTP.ControlSocket
	...
	#tag Method, Flags = &h21
		Private Sub ParseResponse(Data As String)
		  ...
		  If Code > 0 And msg <> "" Then
		    FTPLog(Str(Code) + " " + msg)
		  ElseIf Code > 0 Then
		    FTPLog(Str(Code) + " " + FormatCode(Code).Trim)
		  Else
		    FTPLog(msg)
		  End If
		  ...
		  Case "TYPE"
		    If Code = 200 Then
		      ...
		      Case "A"
		        Me.TransferMode = TransferModes.ASCII
		      Case "L8"
		        Me.TransferMode = TransferModes.Local
		      Case "I"
		        Me.TransferMode = TransferModes.Binary
		      Case "E"
		        Me.TransferMode = TransferModes.EBCDIC
		      End Select
		    End If
	...
	#tag Method, Flags = &h0
		Sub TYPE(Assigns TransferType As FTP.TransferModes)
		  Select Case TransferType
		  Case TransferModes.ASCII
		    DoVerb("TYPE", "A")
		  Case TransferModes.Local
		    DoVerb("TYPE", "L8")
		  Case TransferModes.Binary
		    DoVerb("TYPE", "I")
		  Case TransferModes.EBCDIC
		    DoVerb("TYPE", "E")
		  End Select
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub TYPE(Assigns TransferType As Integer)
		  Select Case TransferModes(TransferType)
		  Case TransferModes.ASCII
		    DoVerb("TYPE", "A")
		  Case TransferModes.Local
		    DoVerb("TYPE", "L8")
